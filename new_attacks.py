...
import time
import math
#Stats declaration (Prevents any weird jank that leads to a breaka)
...
def setenemyfile(x):
    global enemy_name, enemy_hp, enemy_speed, enemy_power, enemy_maxhp, enemy_shape, enemy_stats
    with open(x, 'r') as file:
        enemy_stats = file.readlines()
    enemy_name = str(enemy_stats[0]).rstrip('\n')
    enemy_hp = int(enemy_stats[1])
    enemy_speed = int(enemy_stats[2])
    enemy_power = int(enemy_stats[3])
    wn.addshape(str(enemy_stats[4]).rstrip('\n'))
    enemy_shape = (str(enemy_stats[4]).rstrip('\n'))
    enemy.shape(enemy_shape)
    enemy_maxhp = enemy_hp

def setplayerfile(x):
    global player_name, player_hp, player_speed, player_power, player_maxhp, player_shape, player_stats
    with open(x, 'r') as file:
        player_stats = file.readlines()
    player_name = str(player_stats[0]).rstrip('\n')
    player_hp = int(player_stats[1])
    player_speed = int(player_stats[2])
    player_power = int(player_stats[3])
    wn.addshape(str(player_stats[4]).rstrip('\n'))
    player_shape = (str(player_stats[4]).rstrip('\n'))
